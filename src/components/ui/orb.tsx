...
function Scene({
  ...
  const [seedValue] = useState(
    () => seed ?? Math.floor(Math.random() * 2 ** 32),
  );
  const random = useMemo(() => splitmix32(seedValue), [seedValue]);
  ...
  useFrame((_, delta: number) => {
    ...
    if (modeRef.current === "manual") {
      ...
      if (agentRef.current === null) {
        ...
        targetIn = clamp01(0.8 + Math.sin(t * 5.5) * 0.18);
        targetOut = clamp01(0.9 + Math.sin(t * 4.2) * 0.28);
      } else {
    ...
    const stateSpeedBoost = agentRef.current === "talking" ? 1.6 : 1.0;
    const targetSpeed =
      (0.1 + (1 - Math.pow(curOutRef.current - 1, 2)) * 0.9) * stateSpeedBoost;
  ...
  const uniforms = useMemo(() => {
    return {
      ...
      uInverted: new THREE.Uniform(0), // Updated in useFrame or effect
...
void main() {
   ...
   theta += noise * mix(0.08, 0.45, uOutputVolume);

    // Initialize the base color with Alpha 0 so the empty space becomes transparent
   ...
   float opacity1 = mix(0.2, 0.85, uInputVolume);
float opacity2 = mix(0.15, 0.65, uInputVolume);
    ...
    vec3 color1 = mix(vec3(1.0), vec3(0.0), uInverted); // Bg color
    vec3 color2 = mix(uColor2, uColor1, uInverted);     // Outer Core (darker)
    vec3 color3 = mix(uColor1, uColor2, uInverted);     // Inner Core (lighter)
    vec3 color4 = vec3(1.0);                            // Center Highlight (always white)

    // Convert grayscale color to the color ramp
    float luminance = mix(color.r, 1.0 - color.r, uInverted);
    vec3 finalRgb = colorRamp(luminance, color1, color2, color3, color4); // Apply the color ramp
    
    // Instead of forcing alpha = 1 for the background, we preserve the transparent background properly.
    // Ensure that alpha corresponds to the geometries we drew
    float alphaLayer = max(color.a, totalRingAlpha);
    vec4 outColor = vec4(finalRgb, alphaLayer);
